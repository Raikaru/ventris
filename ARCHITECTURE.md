...
| Game ABI/type recovery | Initial vertical slice | `ventris-game` provides target-specific ABI facts, explicit unknown types, confidence/provenance, repeated base-plus-offset fields, observed `PTRADD` strides, declaration-order nominal fields, symbols, relocations, and user assertions through the library, `recover-types` CLI, Python API, HTTP endpoint, and VS Code command. |
