//! Minimal x86-64 instruction decoding for listing and graph consumers.
//! Native import discovery uses SLEIGH flow in `native::discovery`; this module
//! provides only instruction lengths and control-flow classification.
...
pub enum Flow {
    ...
    JumpCond(u64),
    /// Indirect call (register or memory), with a fallthrough continuation.
    IndirectCall,
